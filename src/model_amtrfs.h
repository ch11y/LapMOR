...
void model_amtrfs( const vector< VD > &  input , const vector< VD > & output, vector< VD > & B , VD & A, double & lambda1 , double& lambda2, int div = 5){
     ...
     //   for( lambda2 = 1 ; lambda2 <= 1 ; lambda2 += 1){
            ...
            for(int start = 0 ; start < n ; start += step ){
              ...
              //  if( cnt ++ > 0 ) break ; 
                ...
                train_amtrfs( train_input, train_output , B , A ,  lambda1 , lambda2 ) ;
                for(int i=0; i < SZ(A) ;++i){
                    cout <<A[i]<<" " ;
                }
                cout << endl; 
                train_error.push_back(  predict_amtrfs( train_input, train_output , B , A , lambda1, lambda2)) ;  
                predict_error.push_back( predict_amtrfs( test_input, test_output , B , A , lambda1, lambda2 )) ;       
   ...
   // }
    ...
    train_amtrfs( input, output,  B , A ,  lambda1, lambda2) ; 
}

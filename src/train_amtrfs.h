...
/*void update_A_Laplacian( const VD& g_B , VD& A , const double & lambda1, const double& lambda2, int K){
   ...
   //  cout <<"update_A"<<endl; 
    for(int i=0 ; i < p ; ++i){ 
        A[i]=0 ;  
        for(int j=0 ; j < q ; ++j){ 
            A[i] = A[i] + fabs(g_B[ j * p + i ]); 
        }
     //   cout <<"A"<< A[i] << endl;
        cout <<  i <<" " << (lambda2 * A[ i ] - lambda1)/K << endl ;
        if( (lambda2*A[i]-lambda1)/ K  < 1  ){ 
            A[i]= 1./500*4;
        }else{
            A[i] = K / ( lambda2*A[i]- lambda1) ; 
        } 
      //  cout <<"A"<<A[i]<<endl; 
    }
...
void update_A_Gaussian(const VD& g_B , VD & A , const double & lambda1 , const double& lambda2, int K){ 
    int p = SZ(A), q = SZ(g_B) / SZ(A) ;  
    for(int i=0 ; i < p ; ++i){
        double x=0 ; 
        for(int j=0; j < q ; ++j){
        ...
        A[i]= (( 2 * lambda1 - lambda2 * x ) + sqrt(  pow(2*lambda1-lambda2*x,2)  + 8 * lambda1*K )) / 4/ lambda1 ; 
        if(A[i] > 1. ){
            A[i]=1.; 
        }
    }
